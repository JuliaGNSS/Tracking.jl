...
function estimate_dopplers_and_filter_prompt(
        ...
        map(estimator_per_system, system_sats.states, sat_sample_params_per_system) do estimator, state, sample_params
            return if sample_params.signal_samples_to_integrate == sample_params.samples_to_integrate_until_code_end
                ...
                dll_discriminator = dll_disc(
                    system_sats.system,
                    filtered_correlator,
                    (state.code_doppler + get_code_frequency(system_sats.system)) / sampling_frequency
                )
                ...
                (estimator = new_estimator, dopplers_and_filtered_prompt = dopplers_and_filtered_prompt)
            else
                (estimator = estimator, dopplers_and_filtered_prompt = DopplersAndFilteredPrompt(
                    0.0Hz,
                    0.0Hz,
                    complex(0.0,0.0)
                ))
    ...
    new_doppler_estimator = ConventionalPLLsAndDLLs(map(x -> map(y -> y.estimator, x), results))
    new_dopplers_and_filtered_prompts = map(x -> map(y -> y.dopplers_and_filtered_prompt, x), results)
