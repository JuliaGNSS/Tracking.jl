...
function estimate_dopplers_and_filter_prompt(
    ...
    new_multiple_system_states = map(
        ...
        return map(first, new_states),
        SystemSatsState(system_sats_state, map(last, new_states))
    end
    return TrackState(
        track_state;
        multiple_system_sats_state = map(last, new_multiple_system_states),
        doppler_estimator = ConventionalPLLAndDLL(
            track_state.doppler_estimator;
            states = map(first, new_multiple_system_states),
        ),
    )
end