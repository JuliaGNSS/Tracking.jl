...
After one warmup call (which seats the `filtered_prompts` buffer's capacity),
steady-state `track!` shows only a small per-call residual (a few hundred
bytes) from closures inside `bit_buffer`'s find-bit logic — short-lived
young-generation allocations that do not trigger GC pauses in the hot SDR
loop.
"""
function track!(
    signal::AbstractVecOrMat,
