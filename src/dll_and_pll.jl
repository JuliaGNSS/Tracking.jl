...
function init_locked_loop(disc, loop_filter, calc_phase, calc_signal, init_phase, init_freq, Δt, sampling_freq)
    phase = calc_phase(init_phase, init_freq, Δt, sampling_freq)
    init_replica = calc_signal(init_phase, init_freq, 1:Δt, sampling_freq)
...
function init_PLL(init_phase, init_freq, Δt, sampling_freq, bandwidth) 
  ...
  calc_signal = GNSSSignals.gen_carrier(1:Δt, init_freq, init_phase, sampling_freq)
  calc_phase = GNSSSignals.get_carrier_phase(Δt, init_freq, init_phase, sampling_freq)    
  #println(disc, loop_filter, calc_phase, calc_signal, init_phase, init_freq, Δt, sampling_freq)
  init_locked_loop(disc, loop_filter, [calc_phase], [calc_signal], [init_phase], [init_freq], Δt, sampling_freq)
end
