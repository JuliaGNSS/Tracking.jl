...
function _set_sat_differential_group_delay(
    ...
    idx = _signal_index(sat.signals, sel...)
    idx == 1 && _check_driver_differential_group_delay(delay)
    idx_tuple = ntuple(identity, length(sat.signals))
...
function set_differential_group_delay!(
    ...
    _warn_differential_group_delay_unread(track_state.doppler_estimator)
...
@inline function _set_differential_group_delay!(sats, sat_id, delay::Maybe{Number}, sel...)
    ...
    # any time unit to the field's `typeof(1.0s)` and refuses everything else with
    # its own message — hence `Maybe{Number}` on every overload above.
