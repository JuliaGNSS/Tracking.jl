...
function _joined_tracking(systems, signals, sample_freqs, beamform, gen_carrier_replicas, gen_code_replicas, carrier_freq_update, code_freq_update, carrier_loop, code_loop, velocity_aidings)
    ...
    #show(STDOUT, "text/plain", joined_correlated_signals)
    #error("test")
...
function _joined_tracking(l1_signals, l5_signals, beamform, l1_gen_carrier_replica, l5_gen_carrier_replica, l1_gen_code_replica, l5_gen_code_replica, carrier_freq_update, code_freq_update, carrier_loop, code_loop, l1_aiding_scale_factor, l5_aiding_scale_factor, velocity_aiding, l1_sampling_freq)
    ...
    (next_l1_signals, next_l5_signals, next_beamform, next_velocity_aiding = 0.0) -> _joined_tracking(next_l1_signals, next_l5_signals, next_beamform, next_l1_gen_carrier_replica, next_l5_gen_carrier_replica, next_l1_gen_code_replica, next_l5_gen_code_replica, next_carrier_freq_update, next_code_freq_update, next_carrier_loop, next_code_loop, l1_aiding_scale_factor, l5_aiding_scale_factor, next_velocity_aiding, l1_sampling_freq), joined_results
end
