...
Fields:
  ...
  - `sample_index`: sample index at which this integration ended, on the time
    ...
    the shared per-chunk origin already gives. A producer that gets it wrong
    there does not break tracking: the fold either lumps every passenger record
    into the chunk's first driver record (losing the exact windows, i.e.
    epoch-smearing) or parks them for the next chunk (a one-chunk lag). The
    loop filters key off `integrated_samples`, so a driver-only or
    single-signal setup never reads it.
"""
