@testset "Satellite state" begin
    ...
    @test get_last_fully_integrated_correlator(state1).accumulators ==
    ...
    @test get_last_fully_integrated_correlator(state2).accumulators ==
          complex.(ones(3) * 2, ones(3) * 3)
    ...
    @test get_last_fully_integrated_correlator(state3).accumulators ==
          complex.(ones(3), zeros(3))
    ...
    state2 = next_system_sats_states[1].states[2]
    state3 = next_system_sats_states[1].states[3]
    @test get_code_phase(state1) == mod(
        10.5 +
        next_system_sats_sample_params[1][1].signal_samples_to_integrate *
        (get_code_frequency(gpsl1) + 0.0Hz) / sampling_frequency,
        1023,
    )
    @test get_code_phase(state2) == mod(
        11.5 +
        next_system_sats_sample_params[1][2].signal_samples_to_integrate *
        (get_code_frequency(gpsl1) + 1.0Hz) / sampling_frequency,
        1023,
    )
    @test get_code_phase(state3) == mod(
        12.5 +
        next_system_sats_sample_params[1][3].signal_samples_to_integrate *
        (get_code_frequency(gpsl1) + 2.0Hz) / sampling_frequency,
        1023,
    )

    @test get_carrier_phase(state1) ≈
          mod2pi(
        2π * (
            0.49 +
            next_system_sats_sample_params[1][1].signal_samples_to_integrate * 0.0Hz /
            sampling_frequency
        ) + π,
    ) - π
    @test get_carrier_phase(state2) ≈
          mod2pi(
        2π * (
            0.8 +
            next_system_sats_sample_params[1][2].signal_samples_to_integrate * 10.0Hz /
            sampling_frequency
        ) + π,
    ) - π
    @test get_carrier_phase(state3) ≈
          mod2pi(
        2π * (
            -0.5 +
            next_system_sats_sample_params[1][3].signal_samples_to_integrate * 21.0Hz /
            sampling_frequency
        ) + π,
    ) - π
