@testset "Downconvert and Correlator" begin
    ...
    downconvert_and_correlator =
        @inferred CPUDownconvertAndCorrelator(system_sats_states, 5000)
    ...
    @test eltype(downconvert_and_correlator.buffers[1][1].carrier_replica_buffer) ==
          ComplexF32
    @test size(downconvert_and_correlator.buffers[1][1].downconvert_signal_buffer) ==
          (5000,)
    @test eltype(downconvert_and_correlator.buffers[1][1].downconvert_signal_buffer) ==
          ComplexF32

    downconvert_and_correlator_f64 =
        @inferred CPUDownconvertAndCorrelator(Float64, system_sats_states, 5000)
    ...
    @test size(downconvert_and_correlator_f64.buffers[1][1].carrier_replica_buffer) ==
          (5000,)
    @test eltype(downconvert_and_correlator_f64.buffers[1][1].carrier_replica_buffer) ==
          ComplexF64
    @test size(downconvert_and_correlator_f64.buffers[1][1].downconvert_signal_buffer) ==
          (5000,)
    @test eltype(downconvert_and_correlator_f64.buffers[1][1].downconvert_signal_buffer) ==
          ComplexF64
    ...
    downconvert_and_correlator_numants2 =
        @inferred CPUDownconvertAndCorrelator(system_sats_states_numants2, 5000)
    @test size(downconvert_and_correlator_numants2.buffers[1][1].code_replica_buffer) ==
          (5004,)
    @test eltype(downconvert_and_correlator_numants2.buffers[1][1].code_replica_buffer) ==
          Int16
    @test size(downconvert_and_correlator_numants2.buffers[1][1].carrier_replica_buffer) ==
          (5000,)
    @test eltype(
        downconvert_and_correlator_numants2.buffers[1][1].carrier_replica_buffer,
    ) == ComplexF32
    @test size(
        downconvert_and_correlator_numants2.buffers[1][1].downconvert_signal_buffer,
    ) == (5000, 2)
    @test eltype(
        downconvert_and_correlator_numants2.buffers[1][1].downconvert_signal_buffer,
    ) == ComplexF32
    ...
    @test eltype(downconvert_and_correlator2.buffers[1][1].carrier_replica_buffer) ==
          ComplexF32
    @test size(downconvert_and_correlator2.buffers[1][1].downconvert_signal_buffer) ==
          (5000,)
    @test eltype(downconvert_and_correlator2.buffers[1][1].downconvert_signal_buffer) ==
          ComplexF32
    ...
    @test eltype(downconvert_and_correlator2.buffers[2][1].carrier_replica_buffer) ==
          ComplexF32
    @test size(downconvert_and_correlator2.buffers[2][1].downconvert_signal_buffer) ==
          (5000,)
    @test eltype(downconvert_and_correlator2.buffers[2][1].downconvert_signal_buffer) ==
          ComplexF32
end
...
@testset "Downconvert and correlate with $type" for type in (:CPU, :GPU)
    ...
    downconvert_and_correlator = @inferred arch_function[type].downconvert_and_correlator(
        system_sats_states,
        num_samples_signal,
    )

    system_sats_sample_params = Tracking.init_sample_params(system_sats_states, 1)
    ...
    @test real.(correlators[1][1].accumulators) ≈ [2921, 4949, 2917] ||
          real.(correlators[1][1].accumulators) ≈ [2921, 4949, 2921]
    ...
    @test real.(correlators[1][2].accumulators) ≈ [2919, 4947, 2915] ||
          real.(correlators[1][2].accumulators) ≈ [2919, 4947, 2919]
end
