...
@testset "correlate_and_downconvert" begin
    ...
    @test downconverted_signals == [[1 + 0im 2 + 0im 3 + 0im], [4 + 0im 5 + 0im 6 + 0im]]

    replica_codes = [[1 1 -1], [-1 -1 1]]
    correlated_signals = Tracking._correlate(downconverted_signals, replica_codes)
    @test correlated_signals[1][1] == [1+0im 2+0im 3+0im; 1+0im 2+0im 3+0im; -1+0im -2+0im -3+0im]
end
