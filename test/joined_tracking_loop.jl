...
@testset "joined_tracking_loop" begin
    ...
    next_joined_tracking_loop, l1_code_phase, l1_carrier_freq_update, gpsl1_prompts_corr_signal,  l5_code_phase, l5_carrier_freq_update, gpsl5_prompts_corr_signal = joined_tracking_loop(l1_incoming_signals, l5_incoming_signals)
...
@testset "estimate SNR" begin
    ...
    beamformed_array = complex(zeros(10))
    fill!(beamformed_array, beamform(gpsl1_prompts_corr_signal)[1])
    beamformed_buffer = CircularBuffer{Complex}(10)
    append!(beamformed_buffer, beamformed_array)
    @test Tracking.estimate_C╱N₀(beamformed_buffer, 10) == 177.40169830895314
