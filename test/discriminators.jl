...
@Testset "DLL discriminator test" begin
    ...
    test_signal_very_late = [1+0.0im,0.5+0.0im,0.0+0.0im]
    
    @test Tracking.pll_disc(test_signal_very_early) == -1
    @test Tracking.pll_disc(test_signal_early) == -0.5
    @test Tracking.pll_disc(test_signal_in_time) == 0
    @test Tracking.pll_disc(test_signal_late) == 0.5
    @test Tracking.pll_disc(test_signal_very_late) == 1
end
