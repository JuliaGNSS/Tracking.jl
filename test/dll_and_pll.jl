@testset "initialize PLL" begin
    ...
    PLL, sampled_carrier, phase = Tracking.init_PLL(1 / 3 * pi, 50, 4000, 4e6, 18.0, 1e-3)
    @test sampled_carrier ≈ cis.(2 * π * 50 / 4e6 * (1:4000) + 1 / 3 * pi) * 1
    @test phase ≈ mod2pi((2 * π * 50 / 4e6) * 4000 + 1 / 3 * pi)
    next_PLL, next_sampled_carrier, next_phase = PLL(correlator_output)
    @test next_phase ≈ mod2pi((2 * π * 50 / 4e6) * 4000 + 1 / 3 * pi)
    @test next_sampled_carrier ≈ cis.(2 * π * 50 / 4e6 * (1:4000) + next_phase) * 1
end
