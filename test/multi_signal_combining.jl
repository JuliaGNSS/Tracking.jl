...
@testset "a non-zero driver delay is refused at construction too" begin
    # The setter is one of two paths that can put a delay on `signals[1]`; a
    # `TrackedSignal` built with one and placed first in the tuple is the other.
    l1cp, l1cd = GPSL1C_P(), GPSL1C_D()
    driver_with_delay = TrackedSignal(l1cp; differential_group_delay = 1.0e-9s)
    @test_throws ArgumentError TrackedSat(
        (driver_with_delay, TrackedSignal(l1cd)),
        7,
        0.0,
        1000.0Hz,
    )
    # `0.0s` and `nothing` restate the definition and pass; a passenger may carry any value.
    for driver_delay in (0.0s, nothing)
        sat = TrackedSat(
            (
                TrackedSignal(l1cp; differential_group_delay = driver_delay),
                TrackedSignal(l1cd; differential_group_delay = -2.0e-9s),
            ),
            7,
            0.0,
            1000.0Hz,
        )
        @test get_differential_group_delay(sat, GPSL1C_D) == -2.0e-9s
    end
end
...
@testset "setting a delay under vector tracking warns where it stops being read" begin
    ...
    @test_logs set_differential_group_delay!(scalar_ts, :gps_l1, 7, GPSL1C_D, 1.0e-9s)
end

@testset "a rebuilt satellite is checked too, not only a freshly built one" begin
    # `TrackedSignal(sig; differential_group_delay = …)` can put a value on a
    # signal that only becomes `signals[1]` when the tuple is handed to
    # `TrackedSat` — a path the setter's own guard never sees, and one the fold
    # itself takes every chunk.
    l1cp, gpsl1 = GPSL1C_P(), GPSL1CA()
    sat = TrackedSat((TrackedSignal(l1cp), TrackedSignal(gpsl1)), 1, 0.0, 1000.0Hz)
    reordered =
        (TrackedSignal(l1cp; differential_group_delay = 1.0e-9s), TrackedSignal(gpsl1))
    @test_throws ArgumentError TrackedSat(sat; signals = reordered)
    # `nothing` and `0.0s` both restate the definition and pass, on either path.
    @test TrackedSat(
        sat;
        signals = (TrackedSignal(l1cp; differential_group_delay = 0.0s), last(reordered)),
    ) isa TrackedSat
end
...
@testset "_combine_with_own is a normalized mean with an exact driver-only path" begin
    ...
    # A zero-weight driver value (e.g. the FLL's first record) drops out exactly.
