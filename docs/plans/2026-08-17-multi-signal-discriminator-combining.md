...
`code_bias` starts at `nothing`, meaning *unknown*, and the caller supplies what
...
# Per-passenger code bias in seconds, relative to the driver, addressed like
# `set_preferred_num_code_blocks_to_integrate!`. The driver's own is 0.0.
set_code_bias!(ts, :gps_l1, 7, GPSL1CA, -1.0e-9)
get_code_bias(sat, GPSL1CA)
```

`code_bias` is a `Maybe{Float64}` field on `TrackedSignal`, so the kwarg-update
...
unset `code_bias` still holds the passenger's code contribution back while its
...
2. `feat(track): per-signal code bias on TrackedSignal`
3. `feat(pll): combine every signal's discriminators into the loop update`
4. `docs: multi-signal discriminator combining and the code bias`
