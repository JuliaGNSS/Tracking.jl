...
The same check runs when a `TrackedSat` is built from `TrackedSignal`s that already carry a delay (`TrackedSignal(sig; differential_group_delay = …)`), so the driver invariant holds on both paths.
